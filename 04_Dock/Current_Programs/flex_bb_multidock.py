...
def run_multi(pdbs):
    global program_dir
    global version
    #os.mkdir(program_dir + "/Flex_bb_Runs/")
    start = time.time()
    current_time = time.time()
    with open("time_bb_flex.txt", "w") as f:
        for pdb in sorted(os.listdir(pdbs)):
            if pdb.endswith(".pdb"):
                prep_dirs(pdb)  # Turn back on
                os.chdir(program_dir + "/Flex_bb_Runs/" + pdb + "/")
                print(pdb)
                ...
                # Run Flex auto
                subprocess.run(["python3", "flexauto_bb_rosetta.py", pdb, version])
                os.chdir(program_dir)
                current_time = time.time()
                f.write(f"{pdb}: {(current_time - start)/3600:.1f} hrs")
                print(f"{pdb}: {(current_time - start)/3600:.1f} hrs")
        f.write(f"Total: {(time.time() - start)/3600:.1f} hrs")
