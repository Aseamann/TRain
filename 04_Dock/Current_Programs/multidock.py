...
def run_multi(pdbs):
    global program_dir
    global version
    os.mkdir(program_dir + "/Runs/")
    start = time.time()
    current_time = time.time()
    for pdb in sorted(os.listdir(pdbs)):
        if pdb.endswith(".pdb"):
            prep_dirs(pdb)
            os.chdir(program_dir + "/Runs/" + pdb + "/")
            print(pdb)
            subprocess.run(["python3", "autodock_rosetta.py", pdb, version])
...
def prep_dirs(pdb):
    global pdb_dir
    global program_dir
    os.mkdir(program_dir + "/Runs/" + pdb + "/")  # Make pdb run file
    # Copy over autodock
    auto = "autodock_rosetta.py"
    copyfile(program_dir + "/" + auto, program_dir + "/Runs/" + pdb + "/" + auto)
    # Copy over pdb file
    copyfile(pdb_dir + "/" + pdb, program_dir + "/Runs/" + pdb + "/" + pdb)
