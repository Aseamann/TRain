...
def cdr_loops():
    trav_file = os.path.join(os.path.dirname(__file__), "trav_alignment_imgt.txt")
    trbv_file = os.path.join(os.path.dirname(__file__), "trbv_alignment_imgt.txt")
    alp = extract_CDRaa(trav_file)
    bet = extract_CDRaa(trbv_file)
    alpha = alp.storeSeqs()
    beta = bet.storeSeqs()
    return(alpha, beta)
